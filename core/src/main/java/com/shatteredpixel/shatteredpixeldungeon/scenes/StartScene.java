...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		title.setPos( (w - title.width()) / 2f, (16 - title.nLines) / 2f);
		align(title);
		add(title);
	...
	private static class SaveSlotButton extends Button {
		...
		protected void layout() {
			...
			if (hero != null){
				...
				name.setPos( hero.x + hero.width() + 6, y + (height - name.nLines)/2f);
				align(name);
				...
				name.setPos( x + (width - name.width())/2f, y + (height - name.nLines)/2f);
				align(name);
			}
