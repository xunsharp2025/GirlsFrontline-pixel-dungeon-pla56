...
public class WndNoel extends Window {
    ...
    public WndNoel(final Noel noel, final Item item ) {
        ...
        RedButton btnWand1 = new RedButton( Noel.Quest.wand1.name() ) {
            @Override
            protected void onClick() {
                selectReward( noel, item, Noel.Quest.wand1 );
            }
        };
        btnWand1.setRect(0, titlebar.top() + titlebar.height() + GAP, WIDTH, BTN_HEIGHT);
        add( btnWand1 );

        RedButton btnWand2 = new RedButton( Noel.Quest.wand2.name() ) {
            @Override
            protected void onClick() {
                selectReward( noel, item, Noel.Quest.wand2 );
            }
        };
    ...
    private void selectReward( Noel noel, Item item, Wand reward ) {
        ...
        if (reward.doPickUp( Dungeon.hero )) {
            ...
            Dungeon.level.drop( reward, noel.pos ).sprite.drop();
        }
